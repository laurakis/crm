...
// header таблицы
export const createHeaderTable =  (table) => {
...
const createTableItem = (user, body) => {
  ...
  actions.changeButton.textContent = 'Change';
  actions.changeButton.addEventListener('click', async () => {
    await modals.mainModal(TYPE.change, user.id);
  });

  actions.deleteButton.textContent = 'Delete';
  actions.deleteButton.addEventListener('click',  () => {

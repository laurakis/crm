...
export async function mainModal(type = TYPE.new, id = '') {
  ...
  if (user.message) {
    const text = document.createElement('p');
    text.textContent = 'User is not find';
    modalContent.append(text);
    return;
  }
  ...
  surNameLabel.innerHTML = 'Surname<span class="symbol">*</span>';
  ...
  firstNameLabel.innerHTML = 'Name<span class="symbol">*</span>';
  ...
  lastNameLabel.innerHTML = 'Patronymic';
  ...
  title.innerHTML = type === TYPE.new
    ? 'New customer' : `Change the data <span class='modal-title__id'>ID: ${user.id}</span>`;
  ...
  cancelButton.textContent = type === TYPE.new ? 'Cancel' : 'Delete customer';
  cancelButton.classList.add('modal__cancel');
